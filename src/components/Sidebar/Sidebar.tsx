...
const CurrentSidebarTabAtoms = new Map<
  SidebarName,
  WritableAtom<SidebarTab, SidebarTab>
>();
...
const SidebarTabs = ({
  ...
  const activeTabAtom = useMemo(() => {
    if (!CurrentSidebarTabAtoms.has(name)) {
      CurrentSidebarTabAtoms.set(name, atom(defaultTab));
    }
    return CurrentSidebarTabAtoms.get(name)!;
  }, [name, defaultTab]);

  const [currentTab, setCurrentTab] = useAtom(activeTabAtom);
...
export const SidebarContext = React.createContext<SidebarName>(null!);
...
export const Sidebar = Object.assign(
  forwardRef((props: SidebarProps, ref: React.ForwardedRef<HTMLDivElement>) => {
    ...
    return (
      <SidebarContext.Provider value={props.name}>
        <SidebarInner {...props} ref={ref} key={props.name} />
